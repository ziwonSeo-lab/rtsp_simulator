...
class NetworkSimulator:
    ...
    def check_tc_support(self):
        """tc 명령어 지원 확인"""
        try:
            result = subprocess.run(['/usr/sbin/tc', 'qdisc'], 
                                  capture_output=True, text=True)
    ...
    def apply_network_conditions(self, stream_id: int, packet_loss: float, 
        ...
        try:
            ...
            netem_params = ['sudo', 'tc', 'qdisc', 'add', 'dev', interface_name, 'root']
            
            # 대역폭 제한이 있는 경우 handle 설정
            if bandwidth_limit > 0:
                netem_params.extend(['handle', '1:', 'netem'])
            else:
                netem_params.append('netem')
    ...
    def cleanup_virtual_interface(self, stream_id: int):
        ...
        if stream_id in self.active_interfaces:
            interface_info = self.active_interfaces[stream_id]
            veth_name = interface_info['veth']
            ...
            try:
                ...
                subprocess.run(['sudo', 'ip', 'link', 'del', veth_name], 
                             capture_output=True)
                
                del self.active_interfaces[stream_id]
                logger.info(f"가상 인터페이스 {veth_name} 정리 완료")
