...
class OverlayRenderer:
    ...
    def create_single_line_overlay(self) -> str:
        """1줄 오버레이 텍스트 생성 (상단 좌측)"""
        current_time = datetime.now()
        
        # GPS 좌표를 60분법으로 변환 (소수점 1자리로 단축)
        lat_dms, lon_dms = format_gps_coordinates(
            self.config.overlay_config.latitude,
            self.config.overlay_config.longitude
        )
        ...
        # 컴팩트한 형식
        overlay_text = (
            f"{self.config.overlay_config.vessel_name} | S{self.config.overlay_config.stream_number:02d} | "
