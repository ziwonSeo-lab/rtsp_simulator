...
class EnhancedFFmpegVideoWriter:
    ...
    def _get_ffmpeg_command(self):
        ...
        # 버벅임 방지를 위한 정확한 타이밍 제어
        cmd.extend(['-r', str(self.fps)])  # 출력 FPS 명시적 설정
        cmd.extend(['-vsync', 'cfr'])      # Constant Frame Rate 강제
        cmd.extend(['-g', str(self.config.keyframe_interval)])
        cmd.extend(['-pix_fmt', self.config.pixel_format])
        cmd.extend(['-fflags', '+genpts'])  # PTS 재생성으로 타이밍 정확성 보장
        cmd.extend(['-avoid_negative_ts', 'make_zero'])  # 음수 타임스탬프 방지
