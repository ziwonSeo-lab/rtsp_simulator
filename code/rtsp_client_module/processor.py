...
class SharedPoolRTSPProcessor:
    ...
    def __init__(self, config: RTSPConfig):
        ...
        for thread_id in range(config.thread_count):
            ...
            self.save_queues[stream_id] = Queue(maxsize=config.save_queue_size)
        
        self.preview_queue = Queue(maxsize=config.preview_queue_size)
        self.stats_dict = self.manager.dict()
        self.stop_event = Event()
        
        # 스트림별 15fps 제어를 위한 공유 딕셔너리와 Lock
        self.shared_stream_last_save_times = self.manager.dict()
        self.stream_timing_lock = self.manager.Lock()
