"""
RTSP 시뮬레이터 워커 프로세스 모듈 - 스트림별 독립 15fps 제어 버전
...
- save_worker_process: 저장 워커 (스트림별 독립적 15fps 제어)

각 워커 프로세스는 멀티프로세싱 환경에서 독립적으로 실행되며,
큐를 통해 데이터를 주고받습니다.

v2.0: 스트림별 독립적 15fps 제어 시스템 구현
- N개 스트림 = N×15fps 총 처리량
- 각 스트림마다 66.67ms 간격으로 독립적 저장
- 정확한 15fps 비디오 출력 보장
"""
...
def save_worker_process(worker_id, save_queue, stats_dict, stop_event, base_output_dir):
    ...
    # =============================================================================
    # 스트림별 독립적인 15fps 제어 시스템
    # =============================================================================
    # 스트림별 마지막 저장 시간 관리
    stream_last_save_times = {}
    
    # 스트림별 15fps 제어 변수
    STREAM_TARGET_FPS = 15.0
    stream_frame_interval = 1.0 / STREAM_TARGET_FPS  # 66.67ms 간격
    
    logger.info(f"🎯 스트림별 독립 15fps 제어 활성화 - 간격: {stream_frame_interval*1000:.1f}ms")
    ...
    try:
        while not stop_event.is_set():
            ...
            # =================================================================
            # 1단계: 새로 들어온 프레임들을 버퍼에 수집
            # =================================================================
            frames_collected = 0
            try:
                while frames_collected < 50:  # 한 번에 최대 50개까지만 처리
                    save_item = save_queue.get_nowait()
                    ...
                    if stream_id not in stream_dirs:
                        ...
                        # 스트림별 타이머 초기화
                        stream_last_save_times[stream_id] = 0.0
                        logger.info(f"🔄 새 스트림 추가: {stream_id}, 독립적 15fps 제어 시작")
                    
                    # 스트림별 15fps 제어 확인
                    time_since_last_save = current_time - stream_last_save_times[stream_id]
                    
                    if time_since_last_save >= stream_frame_interval:
                        # 이 스트림에서 66.67ms가 경과했으므로 저장
                        stream_last_save_times[stream_id] = current_time
                        frame_counts[stream_id] += 1
                        
                        logger.debug(f"📺 {stream_id}: 독립 15fps 저장 "
                                   f"(간격: {time_since_last_save*1000:.1f}ms)")
                        
                        # =============================================================
                        # 비디오 파일로 저장 - 현재 스트림 처리
                        # =============================================================
                        if config.container_format in ['mp4', 'mkv', 'webm', 'avi']:
                            ...
                            # 기존 비디오 writer 종료
                            if selected_stream in video_writers:
                                try:
                                    video_writers[selected_stream].release()
                                    logger.info(f"Stream {selected_stream}: 비디오 저장 완료 - {video_frame_counts[selected_stream]}프레임 "
                                              f"(part{file_counters[selected_stream]:03d})")
                                except Exception as e:
                                    logger.error(f"Stream {selected_stream}: 기존 writer 해제 오류 - {e}")
                                finally:
                                    if selected_stream in video_writers:
                                        del video_writers[selected_stream]
                            
                            # 새 비디오 파일 시작
                            file_counters[selected_stream] += 1
                            video_frame_counts[selected_stream] = 0
                            
                            # 파일명 생성
                            timestamp_str = timestamp.strftime("%Y%m%d_%H%M%S")
                            filename = f"{selected_stream}_{timestamp_str}_part{file_counters[selected_stream]:03d}.{config.container_format}"
                            filepath = os.path.join(stream_dirs[selected_stream], filename)
                            
                            # 비디오 writer 초기화
                            height, width = frame.shape[:2]
                            fps = max(1.0, config.input_fps)
                            
                            logger.info(f"Stream {selected_stream}: 비디오 writer 생성 시작")
                            ...
                            if _check_ffmpeg():
                                try:
                                    logger.info(f"Stream {selected_stream}: Enhanced FFmpeg writer 생성 시도")
                                    video_writers[selected_stream] = EnhancedFFmpegVideoWriter(filepath, fps, width, height, config)
                                    
                                    if video_writers[selected_stream].isOpened():
                                        logger.info(f"Stream {selected_stream}: ✅ Enhanced FFmpeg 비디오 시작 성공 - {filename}")
                                        writer_created = True
                                    else:
                                        raise Exception("Enhanced FFmpeg writer가 열리지 않음")
                                        
                                except Exception as e:
                                    logger.warning(f"Stream {selected_stream}: Enhanced FFmpeg writer 생성 실패 - {e}")
                                    logger.info(f"Stream {selected_stream}: OpenCV VideoWriter로 폴백 시도")
                                    if selected_stream in video_writers:
                                        try:
                                            video_writers[selected_stream].release()
                                        except:
                            ...
                            if not writer_created:
                                logger.info(f"Stream {selected_stream}: OpenCV VideoWriter 생성 시도")
                        ...
                        if stream_id in video_writers and video_writers[stream_id].isOpened():
                            try:
                                ...
                                if success:
                                    ...
                                    if saved_count % 25 == 0:
                                        logger.info(f"Worker {worker_id}: {saved_count}프레임 저장 (스트림별 독립 15fps), "
                        ...
                        # 66.67ms가 아직 경과하지 않았으므로 이 프레임 무시
                        continue
                        
            except queue.Empty:
                time.sleep(0.01)  # 큐가 비어있으면 짧게 대기
                continue
            except Exception as e:
        ...
        # 최종 통계 로그
        logger.info(f"저장 워커 종료 - Worker {worker_id}")
        logger.info(f"  📊 총 저장: {saved_count}프레임 (스트림별 독립 15fps)")
        logger.info(f"  📊 활성 스트림: {len(stream_dirs)}개")
        for stream_id in stream_dirs:
            logger.info(f"  📊 {stream_id}: {frame_counts.get(stream_id, 0)}프레임 저장")